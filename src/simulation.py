...
if __name__ == '__main__':
    ...
    N = 30
    Q = sparse.diags([0.1, 0.0, 0.01]) # e_y的二次项为0.1弱路径跟踪， 直接使用第三项时间cost也可以达到time_opt的目的
    R = sparse.diags([0.0, 0.0])  # v-v_ref的二次项为0,表征弱速度跟踪
    R_lin = np.array([-0.4, 0.0])  # 速度的线性项为-1.0，鼓励更快的速度（负值表示奖励高速度）
    ...
    # 在R_lin = -0.1要配合timecost =0.01使用。不用timecost看起来不会使时间变短
    # 只用速度cost优化器倾向于选择能保持高速度的路径
    # 这通常意味着选择曲率较小的外圈路径
    # 虽然瞬时速度更高，但总路径长度增加，导致总时间反而更长
